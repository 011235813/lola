"""PG training for Escape Room game."""
...
def train(env, *, num_episodes, trace_length, batch_size, gamma,
    ...
    with tf.Session(config=config_proto) as sess:
        ...
        for i in range(num_episodes):
            ...
            aAll = np.zeros((2*env.NUM_ACTIONS))
            j = 0
            ...
            while j < max_epLength:
                ...
                aAll[a_all[0]] += 1
                aAll[a_all[1] + env.NUM_ACTIONS] += 1
                s_old = s
                s = s1
                sP = s1P
                if d == True:
            ...
            if (episodes_run[agent] % batch_size == 0 and
                ...
                last_state0 = np.reshape(np.vstack(trainBatch0[:,3]), [-1, trace_length, env.NUM_STATES])[:,-1,:]
                last_state1 = np.reshape(np.vstack(trainBatch1[:,3]), [-1, trace_length, env.NUM_STATES])[:,-1,:]
            ...
            if len(rList) % summaryLength == 0 and len(rList) != 0 and updated == True:
                ...
                log_items['agent0_C'] = action_prob[0] + action_prob[3]
                log_items['agent0_D'] = action_prob[2] + action_prob[5]
                log_items['agent1_C'] = action_prob[6] + action_prob[9]
                log_items['agent1_D'] = action_prob[8] + action_prob[11]
                if simple_net:
                    ...
                    log_items['pi_1_5'] = np.exp(log_pi0[5][0])
                    ...
                    log_items['pi_2_5'] = np.exp(log_pi1[5][0])
