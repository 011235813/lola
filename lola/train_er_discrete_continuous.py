...
def train(env, *, num_episodes, trace_length, batch_size, gamma,
    ...
    for agent in range(total_n_agents):
        mainQN.append(PolicyDiscreteContinuous(
            'main' + str(agent), agent, env, lr=lr, gamma=gamma,
    ...
    with tf.Session(config=config_proto) as sess:
        ...
        for i in range(num_episodes):
            ...
            while j < max_epLength:
                ...
                # Compute net reward
                for idx_agent in range(n_agents):
                    r[idx_agent] += total_given_to_each_agent[idx_agent]
                    r[idx_agent] -= total_given_by_each_agent[idx_agent]

                total_steps += 1
                for agent_role, agent in enumerate(these_agents):
                    # append the transition to the list that tracks
                    # each agent's episode
                    episodeBuffer[agent_role].append([
                        s[agent_role], a_all[agent_role], r[agent_role], s1[agent_role],
                        d, these_agents[agent_role], list_r_sampled[agent_role],
                        a_all
                    ])
            ...
            # Add each agent's episode to its ExperienceBuffer (see networks.py)
            for agent_role, agent in enumerate(these_agents):
            ...
            if (episodes_run[agent] % batch_size == 0 and
                ...
                for agent_id in range(n_agents):
                    trainBatch.append(buffers[agent_id].sample(
                        batch_size, trace_length, items_per_transition=8))
                    sample_return.append(
                        np.reshape(get_monte_carlo(trainBatch[agent_id][:,2], y, trace_length, batch_size),
                                   [batch_size, -1]))
                    sample_reward.append(
                        np.reshape(trainBatch[agent_id][:,2]- np.mean(trainBatch[agent_id][:,2]),
                                   [-1, trace_length]) * discount)
                    last_state.append(
                        np.reshape(np.vstack(trainBatch[agent_id][:,3]),
                                   [-1, trace_length, env.NUM_STATES])[:,-1,:])
                    # list of 1D arrays
                    list_action_others = list(trainBatch[agent_id][:, 7])
                    action_others_batch.append(
                        util.get_action_others_1hot_batch(list_action_others,
                                                          agent_id, env.NUM_ACTIONS))
                ...
                for idx in range(n_agents):
                    ...
                    feed_dict[mainQN[idx].action_others] = action_others_batch[idx]
                    feed_dict[mainQN[idx].act_cont_sampled] = np.vstack(trainBatch[idx][:, 6])
